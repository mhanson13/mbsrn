...
def _render_managed_deploy_workflow_yaml(
    *,
    ...
    workflow_yaml_unsigned = (
        ...
        "              fi\n"
        "              ingress_neg_convergence=false\n"
        "              ingress_events_output=\"$(mktemp)\"\n"
        "              kubectl describe ingress site-web --namespace \"$K8S_NAMESPACE\" > \"$ingress_events_output\" 2>&1 || true\n"
        "              if grep -qiE 'NEG|network endpoint group|load balancer|loadbalancer|creating|attaching|detaching|sync|provisioning|reconcile' \"$ingress_events_output\"; then\n"
        "                ingress_neg_convergence=true\n"
        "              fi\n"
        "              rm -f \"$ingress_events_output\"\n"
        "              ingress_recently_created=false\n"
        "              ingress_created_at=\"$(kubectl get ingress site-web --namespace \"$K8S_NAMESPACE\" -o jsonpath='{.metadata.creationTimestamp}' 2>/dev/null || true)\"\n"
        "              if [ -n \"$ingress_created_at\" ]; then\n"
        "                ingress_created_epoch=\"$(date -u -d \"$ingress_created_at\" +%s 2>/dev/null || true)\"\n"
        "                now_epoch=\"$(date -u +%s)\"\n"
        "                if [ -n \"$ingress_created_epoch\" ] && [ \"$ingress_created_epoch\" -gt 0 ] && [ \"$now_epoch\" -ge \"$ingress_created_epoch\" ]; then\n"
        "                  ingress_age_seconds=$((now_epoch - ingress_created_epoch))\n"
        "                  if [ \"$ingress_age_seconds\" -le 300 ]; then\n"
        "                    ingress_recently_created=true\n"
        "                  fi\n"
        "                fi\n"
        "              fi\n"
        "              if [ \"$ingress_neg_convergence\" = true ] || [ \"$ingress_recently_created\" = true ]; then\n"
        "                echo \"deploy_runtime_reason_code=ingress_neg_convergence_pending\"\n"
        "              fi\n"
        "              echo \"In-cluster service probe attempt ${probe_attempt}/${probe_max_attempts} failed; waiting for NEG/LB convergence before retrying.\"\n"
...
def _classify_cloudsql_proxy_failure_from_log_text(
    ...
    if "deploy_runtime_reason_code=ingress_neg_convergence_pending" in normalized:
        return _DEPLOY_RUNTIME_REASON_INGRESS_NEG_CONVERGENCE_PENDING, "rollout_verify"
