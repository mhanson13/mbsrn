...
- Managed site runtime image pulls default to private GHCR mode with control-plane-provisioned namespace pull credentials (`ghcr-pull-secret`). Optional public-image mode can disable this requirement only when explicitly configured.
...
- Hybrid deploy-secret bridge model:
  ...
  - managed publish resolves deploy secret source from admin-managed secret first, with runtime env fallback kept only for controlled compatibility.
  ...
  - propagation is guardrailed (approved owner + aligned target tuple + admin/deploy enablement), source-aware (`admin_managed_secret` vs `runtime_env_fallback`), and auditable (`deploy_secret_propagation_attempted/status/reason/source`).
...
- explicit Pages deploy evidence consumption now uses deterministic key precedence (`resolved_live_url` -> `live_url` -> `deployed_url` -> `deploy_result.live_url`) and is applied identically for site-specific and fallback target workflows.
