...
- explicit deploy evidence contract for live URL confirmation:
  ...
  - this ingress contract is generated by MBSRN-managed templates (not operator-authored), and missing/misaligned managed ingress files can leave ingress `ADDRESS` empty even when workload rollout is healthy
