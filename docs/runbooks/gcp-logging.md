...
- `k8s/service.yaml` must include NEG ingress wiring:
  ...
  - `metadata.annotations["cloud.google.com/backend-config"]={"default":"site-web-backend-config"}`
...
- `k8s/ingress.yaml` must include GKE ingress annotations:
  ...
  - `networking.gke.io/managed-certificates: site-web-preview-cert`
...
- `k8s/frontendconfig.yaml` must be present and referenced:
  ...
  - `kind: FrontendConfig`
  - `metadata.name: site-web-frontend-config`
...
3. Confirm health check target matches runtime contract (`/` on port `8080`).
4. Remember: Kubernetes pod readiness and GCLB backend health are separate checks; pods can be `Ready` while GCLB backends remain `UNHEALTHY` if backend health-check wiring is missing/misaligned.
