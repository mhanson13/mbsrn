...
- `k8s/backendconfig.yaml` must exist and define GCLB health checks aligned to the runtime container:
  ...
  - `spec.healthCheck.requestPath: /healthz`
...
3. Confirm health check target matches runtime contract (`/healthz` on port `8080`).
4. Verify runtime health endpoint directly via port-forward:
