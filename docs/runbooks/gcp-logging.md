...
- treat deploy as successful only when all are true for the same site hostname:
  - `dns_record_matches_ingress=true` (where `dns_expected_ip` is reserved static IP when static-IP binding metadata is available)
...
- `DNS matches expected target IP` (Blocked/Pending):
  - UI fields: `dns_record_matches_ingress`, `dns_expected_ip`, `dns_observed_ip`, `ingress_ip`, `expected_static_ip_address`, `ingress_status_ip`, `ingress_status_ip_matches_static_ip`, `static_ip_status`, `static_ip_users`, `static_ip_bound_to_expected_forwarding_rule`
  - reason codes: `dns_record_mismatch`, `dns_points_to_old_ingress_ip`, `ingress_ip_assigned_but_dns_not_updated`, `ingress_status_ip_stale_or_mismatched` (advisory)
...
- DNS expected-target evaluation prefers reserved static-IP binding evidence when available:
  - when ingress annotation matches expected per-site static-IP name, workflow fetches `gcloud compute addresses describe <name> --global --format="yaml(address,status,users)"`.
  - if static IP binding is healthy (`IN_USE` + expected forwarding-rule user tokens), reserved static `address` becomes `dns_expected_ip`.
  - ingress status IP may lag behind and is advisory in this case.
...
- `k8s/ingress.yaml` must include GKE ingress annotations:
  ...
  - runtime workflow emits static-IP binding diagnostics:
...
- `workflow_run_failure_reason_code=dns_record_mismatch` / `dns_points_to_old_ingress_ip` / `ingress_ip_assigned_but_dns_not_updated`
  ...
  - treat as propagation delay, resolver visibility lag, or out-of-band DNS mutation; compare `dns_expected_ip`, `dns_observed_ip`, `ingress_ip`, static-IP binding fields, and latest `seo_migration_managed_site_dns_ensure` event.
- `workflow_run_failure_reason_code=ingress_status_ip_stale_or_mismatched`
  - advisory/static metadata lag: ingress status IP differs from reserved static IP while static-IP binding evidence is healthy and DNS matches reserved static IP.
  - do not treat this code alone as DNS failure; continue validating HTTPS and certificate identity gates.
