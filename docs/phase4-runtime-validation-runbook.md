...
- Kubernetes `mbsrn-secrets` is populated with required secret values.
...
```bash
kubectl get ns mbsrn-dev
kubectl get ns mbsrn
```

### 4.3 Verify rollout health
```bash
NAMESPACE=mbsrn-dev   # or mbsrn
kubectl -n "$NAMESPACE" get deploy mbsrn-api mbsrn-ui
kubectl -n "$NAMESPACE" rollout status deploy/mbsrn-api --timeout=5m
kubectl -n "$NAMESPACE" rollout status deploy/mbsrn-ui --timeout=5m
kubectl -n "$NAMESPACE" get pods -l app=mbsrn-api
kubectl -n "$NAMESPACE" get pods -l app=mbsrn-ui
```

### 4.4 Verify deployed image refs
```bash
kubectl -n "$NAMESPACE" get deploy mbsrn-api -o jsonpath='{.spec.template.spec.containers[0].image}{"\n"}'
kubectl -n "$NAMESPACE" get deploy mbsrn-ui -o jsonpath='{.spec.template.spec.containers[0].image}{"\n"}'
```
...
`mbsrn-migrate-<github_run_id>-<github_run_attempt>`.
...
```bash
kubectl -n "$NAMESPACE" get configmap mbsrn-config -o yaml
```
...
Confirm `REDIS_URL` exists in `mbsrn-secrets` (do not expose secret value in shared logs/screenshots).
...
```bash
kubectl -n "$NAMESPACE" logs deploy/mbsrn-api --tail=200
```

### 5.3 Validate fail-closed behavior (dev namespace exercise)
Perform this only in non-production:

1. Intentionally break Redis connectivity in `mbsrn-dev` (for example invalid `REDIS_URL` in dev secret).
...
- dev: `dev.mbsrn.example.com`
- prod: `mbsrn.example.com`

```bash
kubectl -n "$NAMESPACE" get ingress mbsrn-ingress -o yaml
```
...
```bash
API_BASE=https://dev.mbsrn.example.com   # or https://mbsrn.example.com
curl -i -H "Origin: https://dev.mbsrn.example.com" "$API_BASE/health"
...
- `kubectl describe deploy` and pod logs for `mbsrn-api`/`mbsrn-ui`.
...
Rollback baseline:
```bash
kubectl -n "$NAMESPACE" rollout history deploy/mbsrn-api
kubectl -n "$NAMESPACE" rollout undo deploy/mbsrn-api
kubectl -n "$NAMESPACE" rollout undo deploy/mbsrn-ui
```
