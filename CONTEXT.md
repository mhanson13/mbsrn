...
The first shared-edge implementation slice is now code-complete behind `managed_preview_endpoint.gateway_api_enabled`, which defaults to false and rejects incomplete configuration. Platform bootstrap can idempotently create the global address, DNS authorization record, Google-managed wildcard certificate, certificate map/entry, enable the Gateway API, and apply the HTTPS-only Gateway without changing a site A record. When explicitly enabled, site publication adds a separate `site-web-gateway` Service, `HealthCheckPolicy`, and exact-host `HTTPRoute` alongside the existing Ingress resources. The separate Service is required because GKE does not allow one Service to be referenced by both Ingress and Gateway during coexistence. Current release verification still uses the legacy endpoint; shared-edge readiness and DNS cutover are subsequent slices.
