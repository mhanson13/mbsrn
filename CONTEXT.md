...
The shared Gateway migration is implemented behind a disabled feature flag but is not active in production. Current live previews still use per-site Ingress, FrontendConfig, BackendConfig, static-IP, and certificate bindings. These resources must be migrated by coexistence and verified cutover rather than edited in place without rollback.

The shared-edge implementation is code-complete behind `managed_preview_endpoint.gateway_api_enabled`, which defaults to false and rejects incomplete configuration. Platform bootstrap can idempotently create the global address, DNS authorization record, Google-managed wildcard certificate, certificate map/entry, enable the Gateway API, and apply the HTTPS-only Gateway without changing a site A record. When explicitly enabled, site publication adds a separate `site-web-gateway` Service, `HealthCheckPolicy`, and exact-host `HTTPRoute` alongside the existing Ingress resources. The separate Service is required because GKE does not allow one Service to be referenced by both Ingress and Gateway during coexistence. Gateway-mode deploy now performs provider-backed, read-only Certificate Manager, Compute, and Kubernetes checks before site DNS reconciliation; pending or unavailable platform state blocks without changing DNS and never invokes the per-site certificate path. The workspace replaces self-managed certificate controls with four concise shared-edge checks, while sanitized resource evidence remains administrator-only. Production bootstrap, canary route validation, DNS cutover, and legacy cleanup remain pending.

The first production bootstrap attempt created the shared global address, DNS authorization/CNAME, wildcard certificate, certificate map, and map entry, then stopped before applying the Gateway because the renderer imported the full API package in a clean Cloud Shell. The renderer is now dependency-free, and Gateway API channel detection uses the cluster's actual `networkConfig.gatewayApiConfig.channel` field. Repeating the same bootstrap command safely reuses the completed resources and resumes at Gateway reconciliation.
