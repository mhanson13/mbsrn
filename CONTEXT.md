...
2. Self-managed certificate generation reaches the API but the `mbsrn-api` workload identity lacks `secretmanager.secrets.create`. It also lacks `compute.sslCertificates.create`, so certificate publication would fail after vault creation.
...
Preview TLS capability checks now test the runtime principal's required Secret Manager and Compute permissions before key generation. Production has a narrow `mbsrnPreviewTlsOperator` custom role bound to the API workload identity. Generate/import and `ensure` operations remain API-side so private keys do not transit GitHub.
